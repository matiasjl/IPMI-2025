...
void mouseClicked() {
  ...
  if (dist(mouseX, mouseY, botonNextPosX, botonNextPosY) < radioBoton) {
    if (isPlayingAnimationNext == false && pantallaActual < 4) {
...
void MostrarBotones() {
  //Matemáticas para la animación de botones
  pushStyle();
  radioBoton = 25;
  ...
  //Botones Idle: sin interactuar
  tint(255, 80);
  imageMode(CENTER);
  image(botonBack, botonBackPosX, botonBackPosY);
  image(botonNext, botonNextPosX, botonNextPosY);

  //Botones Hover: Interaccion de botones si el mouse esta sobre ellos
  if (dist(mouseX, mouseY, botonBackPosX, botonBackPosY) < radioBoton) {
    tint(255, 255);
    image(botonBack, botonBackPosX, botonBackPosY);
  }
...
void AnimacionDeSubir() {
  if (isPlayingAnimationNext == true) {
...
void AnimacionDeBajar() {
  if (isPlayingAnimationBack == true) {
    if (coordenadaY0 >= ultimaCoordenadaY0 + height) {
